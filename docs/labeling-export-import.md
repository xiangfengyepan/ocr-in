...
| `kind` | string | no | `word` \| `line`. Routes the sample to the CRNN (word) or TrOCR (line) training set. Default `word`. |
| `confidence` | number \| null | no | Recognizer confidence in `[0,1]` for imported/OCR samples; `null` for hand-drawn words. Drives lowest-confidence-first review ordering. |

Example (`manifest.jsonl`, two samples):

```json
{"text": "hello", "language": "english", "rating": "correct", "engine_guess": "helo", "kind": "word", "confidence": 0.82, "image": "images/english/1.png"}
{"text": "café", "language": "spanish", "rating": "incorrect", "engine_guess": "cafe", "kind": "line", "confidence": null, "image": "images/spanish/2.png"}
```
...
- Coercion for robustness:
  ...
  - `kind` missing → stored as `word`; `confidence` missing → stored as `null`.
